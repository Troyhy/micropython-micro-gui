...
modestr = {RGB565: "16-bit color RGB565", GS8: "8-bit color RRRGGGBB", GS4_HMSB: "4-bit greyscale",
           RGB565_I: "16-bit color RGB565 (Inverted, Big)"}
fmtstr = {RGB565: b"P6",RGB565_I: b"P6", GS8: b"P6", GS4_HMSB: b"P5"}  # Netbpm file ID strings
...
def convrgb(arr, rows, cols, si, so, bits, rgb565_i = False):
    red = []
    grn = []
    blu = []
    for row in range(rows):
    ...
    if bits == 8:
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                bytes = int.to_bytes(op, 2, "big")
                if rgb565_i:
                    bytes = int.to_bytes(transform_palette(op), 2, "little")

                so.write(bytes)  # Red first
...
def transform_palette(color565):
    ...
    output = (color & 0xFF00) >> 8 | color  << 8
    # invert bitwise
    return (output ^ 0xffff) & 0xffff
...
def conv(arr, si, so, height, width, mode):
    ...
    elif mode == RGB565_I:  # 16-bit color
        print("inverted RGB565")
        convrgb(arr, rows, cols, si, so, 16,rgb565_i=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-I",  "--invert-rgb565", action="store_true", help="invert value and use Big endian")
    ...
    if extension == ".PPM":
        if args.rgb565:
            mode = RGB565_I if args.invert_rgb565 else RGB565
        else:
            mode = GS8  # Color image 16/8 bits
